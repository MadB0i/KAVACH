...
use crate::types::{
    ...
    ApprovalStoreConfig, ConsumedApproval, MAX_APPROVAL_SUMMARY_LENGTH, MAX_APPROVAL_TTL_SECONDS,
    PendingApproval,
};
...
impl ApprovalBroker for SqliteApprovalBroker<SqliteApprovalStore> {
    fn request_approval(
        ...
        let ttl = ttl_seconds
            .unwrap_or(self.store.default_ttl_seconds())
        ...
        if pending_count >= self.store.max_pending() as u64 {
    ...
    fn list_pending(&self, limit: Option<u64>) -> Result<Vec<ApprovalRecord>, ApprovalError> {
        ...
        rows.into_iter().map(crate::store::row_to_record).collect()
    }
    ...
    fn approve(
        ...
        let matched_rule_ids = parse_rule_ids(&row.matched_rule_ids)?;
    ...
    fn consume(
        ...
        // Validate persisted request binding before irreversibly consuming the approval.
        let digest_bytes = hex::decode(&row.request_digest_hex).map_err(|e| {
            ApprovalError::database_corruption(format!("invalid request digest: {e}"))
        })?;
        let request_digest: [u8; 32] = digest_bytes
            .as_slice()
            .try_into()
            .map_err(|_| ApprovalError::database_corruption("invalid request digest length"))?;
    ...
    fn get_approval(&self, approval_id: &ApprovalId) -> Result<ApprovalRecord, ApprovalError> {
        let row = self
            ...
            .load(approval_id.as_str())?
            .ok_or_else(ApprovalError::not_found)?;
        crate::store::row_to_record(row)
    }
}

fn parse_rule_ids(value: &str) -> Result<Vec<String>, ApprovalError> {
    let rule_ids: Vec<String> = serde_json::from_str(value).map_err(|e| {
