...
export default function SecurityWarnings() {
  ...
  return (
    <div className="page">
      <h2 className="page__title">Security Warnings</h2>

      {warnings.length === 0 ? (
        <EmptyState icon={'\u2713'} title="No Security Warnings" description="All clear. No security warnings have been recorded." />
      ) : (
          ...
          {warnings.map((w) => (
            <div key={`${w.sequence}-${w.timestamp}`} className="warning-card">
              <div className="warning-card__header">
                ...
                <span className="warning-card__time">{w.timestamp ? new Date(w.timestamp).toLocaleString() : '?'}</span>
              </div>
