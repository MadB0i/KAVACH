...
import { getApprovals, getAuditEvents, getPolicies, getHealth, getReady, getStatus } from '../api';
import type { ApprovalRecord, AuditEvent, PolicyInfo, HealthStatus, ReadyStatus, StatusInfo } from '../types';
...
export default function Overview() {
  ...
  const fetchAll = useCallback(async () => {
    const fetches = [
      ...
      { key: 'audit', fn: getAuditEvents({ limit: 100 }), setter: setAuditEvents },
      { key: 'policies', fn: getPolicies(), setter: setPolicies },
      { key: 'health', fn: getHealth(), setter: setHealth },
      { key: 'ready', fn: getReady(), setter: setReady },
  ...
  const healthOk = (health?.status === 'ok' || health?.status === 'healthy') && ready?.ready === true;
  ...
  return (
    <div className="page">
      <PageHeader
        ...
        subtitle={
          statusInfo
            ? `${statusInfo.service} v${statusInfo.version || '?'} \u2014 Uptime: ${statusInfo.uptime_seconds !== undefined ? `${Math.floor(statusInfo.uptime_seconds / 60)}m` : '?'}`
            : undefined
        }
      />
      ...
      <div className="metric-grid">
        ...
        <MetricCard
          ...
          subtitle="Latest 100 audit events"
        />
        <MetricCard
      ...
      <div className="overview-grid">
        <SectionCard title="Recent Activity">
          {ls.audit?.loading ? (
            ...
            <div className="inline-empty">No recent audit events</div>
          ) : (
        ...
        <SectionCard title={`Pending Approvals (${pendingCount})`}>
          {ls.approvals?.loading ? (
            ...
            <div className="inline-empty">All clear — no pending approvals</div>
          ) : (
            <div className="audit-mini-list">
              {approvals
